...

{
    ...
    public function setAtivacao($id, $idcliente, $idusuario, $status, $statusemail) {       


        $sql = "INSERT INTO mi_ativacao (Id, Idcliente, Idusuario, Status, Statusemail, Datacriacao) VALUES (?,?,?,?,?, Now())";
        $stmt = $this->mysqli->prepare($sql); 
        $stmt->bind_param("iiiii", $id, $idcliente, $idusuario, $status, $statusemail);
    ...
    public function setAtivacaoProduto($idativacao, $idproduto, $idmodalidade, $chave) { 
        
        $seq = 1;

        $sql = "INSERT INTO mi_ativacao_produto (Idativacao, Idproduto, Idmodalidade, Chave) VALUES (?,?,?,?)";
        $stmt = $this->mysqli->prepare($sql); 
        $stmt->bind_param("iiis", $idativacao, $idproduto, $idmodalidade, $chave); 
    ...
    public function updateAtivacao($id, $idcliente)

    {

        $stmt = $this->mysqli->prepare("UPDATE mi_ativacao SET Idcliente = ?, Dataatualizacao = Now() WHERE Id = ?");
    ...
    public function pesquisaClienteAtivacao($cliente, $data, $status)

    {
        ...
        $filtro = " FROM mi_ativacao

                                            JOIN cd_cliente ON (cd_cliente.Id = mi_ativacao.Idcliente)

                                            JOIN cd_usuario ON (cd_usuario.Id = mi_ativacao.Idusuario)
                                        
                                            JOIN cd_estado ON (cd_estado.Id = cd_cliente.Idestado)
                                            
                                            JOIN cd_pais ON (cd_pais.Id = cd_cliente.Idpais)
                                        
                                            JOIN mi_ativacao_produto ON (mi_ativacao_produto.Idativacao = mi_ativacao.Id)	
                                            
                                            JOIN mi_produto ON (mi_produto.Id = mi_ativacao_produto.Idproduto)

                                            WHERE coalesce(cd_cliente.Nomefantasia, '') like '%$cliente%'

                                               AND coalesce(cd_usuario.mascara, '') like '$mascara%'                                               

                                               AND ((mi_ativacao.Datacriacao = '$data') or ('$data' = '-1'))

                                               AND ((mi_ativacao.Status = '$status') or ('$status' = '-1'))

                                            ORDER BY mi_ativacao.Id desc";
        ...
        $sqlConsulta = "SELECT 
                                mi_ativacao.Id, cd_cliente.Id as IdCliente, cd_cliente.Nomefantasia, cd_cliente.Razaosocial, cd_cliente.Cnpjcpf, cd_cliente.Email as Emailcliente,
                            
                                cd_cliente.Telefone, cd_cliente.Celular, cd_cliente.Endereco, cd_cliente.Cep, cd_cliente.Bairro, cd_cliente.Complemento, cd_cliente.Cidade, cd_cliente.IdEstado, cd_cliente.IdPais,
                            
                                mi_ativacao.Status, mi_ativacao.StatusEmail, mi_ativacao.Datacriacao, cd_usuario.Nome as Vendedor,
                            
                                CASE mi_ativacao.Status WHEN 1 THEN 'Contato com o cliente realizado' WHEN 2 THEN 'Implantação Agendada' WHEN 3 THEN 'Implantação Finalizada' WHEN 4 THEN 'Solicitação Cancelada' else 'Solicitação Recebida' END AS StatusDescricao,
                            
                                CASE StatusEmail WHEN 0 THEN 'Enviado' else 'Reenviado' END AS StatusEmailDescricao, cd_estado.Nome as Estado, cd_pais.nome as Pais, mi_ativacao.Dataatualizacao, mi_produto.Descricao as produto, mi_ativacao_produto.Chave"
