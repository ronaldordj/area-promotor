...
$resultado = $banco->getEmail("A");
	...
	$mail->From       = $Remetente;
	$mail->FromName   = utf8_decode("ALTERADA - Venda InOptimus");
	$mail->Subject    = utf8_decode("ALTERADA Venda InOptimus chave ".$chave." - Movel IN");	
	$mail->WordWrap   = 50; // set word wrap

	$mail->MsgHTML(utf8_decode($body));	

	$mail->AddReplyTo($emailusu, utf8_decode("Venda InOptimus chave ".$chave." - Movel IN"));
	...
	if (!$mail->Send()) {
		...
		echo "<script>location.href='enviarClienteRen.php?id=$atv'</script>";
	}